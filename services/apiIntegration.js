  ...
  try {
    ...
    for (const keyword of searchKeywords) {
      ...
      try {
        ...
        console.error(`Error fetching Walmart products for keyword "${keyword}":`, error)
      }
    }
  ...
  try {
    ...
    const stores = await Store.find({
      $or: storeNames.map((name) => ({
        $or: [{ name: { $regex: name, $options: "i" } }, { chainName: { $regex: name, $options: "i" } }],
      })),
    })
    ...
    for (const product of products) {
      ...
      const newProduct = {
        ...
        type: product.name.toLowerCase().includes("great value") ? "Store Brand" : "National Brand",
        size: product.size || "",
        ...
        isBulkOption: product.name.toLowerCase().includes("pack") || Math.random() > 0.8, // 20% chance of being bulk
        isSeasonalProduce: product.category === "Produce" && Math.random() > 0.7, // 30% chance for produce
        ...
        priceRange: {
          min: product.price ? product.price * 0.8 : generateRandomPrice() * 0.8,
          max: product.price ? product.price * 1.2 : generateRandomPrice() * 1.2,
          period: 6,
        },
    ...
    if (priceUpdates.length > 0) {
      ...
      for (const update of priceUpdates) {
        const product = allProducts.find((p) => p._id.toString() === update.productId.toString())
        if (!product) continue
        ...
        if (!product.currentLowestPrice.price || update.price < product.currentLowestPrice.price) {
...
function determineCategoryFromName(name, keyword = "") {
  ...
  if (
    ...
    return "Meat"
  } else if (lowerName.includes("bread") || lowerName.includes("bakery") || lowerName.includes("bun")) {
    return "Bakery"
